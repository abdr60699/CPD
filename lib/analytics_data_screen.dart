...
class _AnalyticsDataScreenState extends State<AnalyticsDataScreen>
  ...
  Future<void> _loadMostClickedProperties() async {
    final response = await _supabase.rpc('get_most_clicked_properties_data');
    _mostClickedProperties = List<Map<String, dynamic>>.from(response ?? []);
  }

  // 2. Click Trends (7 days)
  Future<void> _loadClickTrends() async {
    final response = await _supabase.rpc('get_click_trends_7_days');
    _clickTrends = List<Map<String, dynamic>>.from(response ?? []);
  }

  // 3. Property Type Performance
  Future<void> _loadPropertyTypePerformance() async {
    final response = await _supabase.rpc('get_property_type_performance');
    _propertyTypePerformance = List<Map<String, dynamic>>.from(response ?? []);
  }

  // 4. Hourly Click Patterns
  Future<void> _loadHourlyPatterns() async {
    final response = await _supabase.rpc('get_hourly_click_patterns');
    _hourlyPatterns = List<Map<String, dynamic>>.from(response ?? []);
  }

  // 5. User Engagement
  Future<void> _loadUserEngagement() async {
    final response = await _supabase.rpc('get_user_engagement_metrics');
    _userEngagement = List<Map<String, dynamic>>.from(response ?? []);
  }

  // 6. Conversion Funnel
  Future<void> _loadConversionFunnel() async {
    final response = await _supabase.rpc('get_conversion_funnel_data');
    _conversionFunnel = List<Map<String, dynamic>>.from(response ?? []);
  }

  // 7. Device Analytics
  Future<void> _loadDeviceAnalytics() async {
    final response = await _supabase.rpc('get_device_analytics_data');
    _deviceAnalytics = List<Map<String, dynamic>>.from(response ?? []);
  }
